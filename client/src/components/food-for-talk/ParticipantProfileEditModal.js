...
const ParticipantProfileEditModal = ({ isOpen, onClose, onProfileUpdated }) => {
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      const response = await apiService.updateFoodForTalkProfile(editData);
      setProfile(response.participant);
      toast.success('Profile updated successfully');
      if (onProfileUpdated) {
        onProfileUpdated(response.participant);
      }
      onClose();
    } catch (error) {
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-gradient-to-br from-purple-900 via-blue-900 to-indigo-900 rounded-2xl border border-white/20 max-w-4xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between p-6 border-b border-white/20">
          <h2 className="text-2xl font-bold text-white">Edit My Profile</h2>
...
export default ParticipantProfileEditModal;
